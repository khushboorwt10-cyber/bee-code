...
class PromotionController extends GetxController {
  // ✅ Fix: initialize directly — no more `late`, no more LateInitializationError
  final RxString selectedGoal = 'Get a promotion'.obs;

  @override
  void onInit() {
    super.onInit();
    // Override with navigation argument if one was passed
    final arg = Get.arguments as String?;
    if (arg != null && arg.isNotEmpty) {
      selectedGoal.value = arg;
    }
  }
  ...
  RxList<Course> get filteredCourses {
    ...
    if (selectedProgramTypes.isNotEmpty) {
      result = result
          .where((c) => selectedProgramTypes.contains(c.programType))
          .toList();
    }
    if (selectedUniversities.isNotEmpty) {
      result = result
          .where((c) => selectedUniversities.contains(c.university))
          .toList();
    }
    if (selectedDurations.isNotEmpty) {
      result = result.where((c) {
        for (final d in selectedDurations) {
          if (d == "Less than 6 months" &&
              _monthsFromDuration(c.duration) < 6) return true;
          if (d == "6 - 12 months") {
          ...
          if (d == "More than 2 years" &&
              _monthsFromDuration(c.duration) > 24) return true;
        }
        return false;
      }).toList();
    }
    ...
    if (selectedSort.value == "Duration - low to high") {
      result.sort((a, b) =>
          _monthsFromDuration(a.duration)
              .compareTo(_monthsFromDuration(b.duration)));
    } else if (selectedSort.value == "Duration - high to low") {
      result.sort((a, b) =>
          _monthsFromDuration(b.duration)
              .compareTo(_monthsFromDuration(a.duration)));
    }
  ...
  int _monthsFromDuration(String dur) {
    ...
    if (dur.contains("yr") || dur.contains("year")) {
      final n =
          double.tryParse(dur.replaceAll(RegExp(r'[^0-9.]'), '')) ?? 1;
      return (n * 12).toInt();
    }
  ...
  bool isOptionSelected(String tab, String option) {
    switch (tab) {
      case "Area of interest":
        return selectedAreas.contains(option);
      case "Program Type":
        return selectedProgramTypes.contains(option);
      case "University":
        return selectedUniversities.contains(option);
      case "Duration":
        return selectedDurations.contains(option);
      default:
        return false;
    }
  }
  ...
  void toggleOption(String tab, String option) {
    switch (tab) {
      case "Area of interest":
        toggleAreaOption(option);
        break;
      case "Program Type":
        toggleProgramType(option);
        break;
      case "University":
        toggleUniversity(option);
        break;
      case "Duration":
  ...
  int countForTab(String tab) {
    switch (tab) {
      ...
      case "Program Type":
        return selectedProgramTypes.length;
      case "University":
